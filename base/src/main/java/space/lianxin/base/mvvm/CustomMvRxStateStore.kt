 ...
 *
 */
class CustomMvRxStateStore<S : Any>(initialState: S) : MvRxStateStore<S> {
    /**
    ...
    @Volatile
    private var currentThreadState: Int = THREAD_ALIVE

    /**
     * When error happened, user need to handle  error with this block
     */
    var errorHandleCallback: ((e: Throwable) -> Unit)? = null
    ...
    override fun get(block: (S) -> Unit) {
        ...
        if (currentThreadState == THREAD_DEAD) {
            initFlushQueueSubject()
        }
    ...
    private fun handleError(throwable: Throwable) {
        ...
        currentThreadState = THREAD_DEAD
        errorHandleCallback?.invoke(throwable)
    ...
    private fun initFlushQueueSubject() {
        currentThreadState = THREAD_ALIVE
    ...
    companion object {
        ...
        private const val THREAD_ALIVE = 1

        private const val THREAD_DEAD = 0
    }
}
