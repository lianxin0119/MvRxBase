...
/**
 * Create by YangYang on 2018/10/12 10:21.
 */
class RoundedCornersTransformation @JvmOverloads constructor(
    ...
    override fun transform(
        ...
        val newBitmap = zoomBitmap(pool, toTransform, outWidth, outHeight)
        ...
        paint.shader = BitmapShader(newBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP)
    ...
    private fun drawCircle(canvas: Canvas, paint: Paint, right: Float, bottom: Float) {
        val radius = if (right <= bottom) {
            right / 2.0f
        } else {
            bottom / 2.0f
        }
        canvas.drawCircle(right / 2.0f, bottom / 2.0f, radius, paint)
    }
